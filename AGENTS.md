...
2. Shared components follow the shadcn philosophy: owned code, children + `className`, no prop-per-feature APIs. Nothing new becomes "shared" until a second widget concretely needs it. Everything shared lives in `src/wigl/` behind exactly three barrels — visual/layout primitives from `@/wigl`, stateful/React hooks from `@/wigl/hooks`, plain non-React helpers from `@/wigl/utils` — each barrel's `index.ts` is the authoritative list of what it exports; widgets never deep-import past those three. The header component's own content (title, buttons) is ordinary interactive/selectable content — only its small top-left grip drags the widget; use `data-no-drag` for custom clickable elements placed inside the grip itself (rare), never `stopPropagation` workarounds.
