...
`loadPlugins()` discovers installed widget folders at startup (below); the folder name *is* the widget's **base** id (used as its grid-layout key and its `useStorage`/`useQuery` key prefix — nowhere else declares it), not a window label — a widget renders as a grid item inside whichever monitor's window it's assigned to, it doesn't get its own OS window (see `docs/architecture.md`). Don't name a folder `main` (the hidden bootstrap window) or `wigl` (the app's name). **Adding a widget = creating the folder + `bun run widget:install`. Deleting it = `bun run widget:rm <id>`. No registration, no config edits, nothing else in the app itself.**

**A widget folder can run as more than one instance.** Right-click a widget's header → "Duplicate" (hidden unless `wigl.instantiable` allows it, see above) mounts a second, independent copy at the next open grid slot — its own position, its own `useStorage`/`useQuery` rows, its own `require` scope, all keyed by a generated instance id rather than the folder id. The always-present first instance keeps its id equal to the folder id (nothing about a single-instance widget's storage/layout changes), so this is purely additive: a widget's own code never needs to know or care whether it's the base instance or a duplicate — `useStorage("thing")` isolates itself automatically either way, the same as it always has across two *different* widgets. A duplicate is **session-temporary**: its instance id lives in `src/App.tsx` React state (seeded empty each launch, broadcast monitor-to-monitor over the `wigl-duplicates` event, never persisted), so closing it *erases* it (drops the instance and its `widget_layout` entry — its `useStorage`/`useQuery` rows are left as inert orphans, the hash id never recurs) and quitting wigl drops every duplicate. `src/wigl/plugins/loader.ts` (`loadOne`'s `instanceId` param, and `loadPlugins`'s `extraInstances`) and `src/wigl/plugins/instances.ts` (`generateInstanceId`) are the mechanism; `src/wigl/Desktop.tsx`'s right-click menu is the only UI for it.
...
- **`wigl.instantiable`** — whether a second (third, ...) copy of this widget may be created via the desktop's right-click "Duplicate" entry (see below). Defaults to `true`; set `false` if the widget assumes something no two mounts can safely share — a fixed server port, a lock file keyed by something not scoped per instance, anything that isn't already routed through `useStorage`/`useQuery`. `wigl-widgets/LocalCode/package.json` is a real example, with the reasoning in that widget's own `AGENTS.md`.
...
**Heavy libraries several widgets need live in the host, not in each bundle.** A widget's build inlines its own npm deps, so a big one (a WYSIWYG markdown editor) would be duplicated across every widget that used it. Once a second widget needs it, it's promoted to a component in `src/wigl/` behind `@/wigl` (lazy-loading its own libs, so nothing is fetched until a widget mounts it) and the widgets drop the dep. A widget never adds such a dep itself when the barrel already serves it — check `src/wigl/index.ts`.
