...
**Widgets arrive from two places, and the window layer doesn't care which.** Builtins are compiled in via `import.meta.glob`; plugins are read off disk at startup, evaluated in the same JS realm, and merged into the same id→component map before `<Desktop>` mounts (`src/wigl/plugins/`, contract in `docs/plugins.md`). Neither is a window, neither touches Rust, and a plugin gets its React and every shared helper from the host through a module registry rather than bundling its own — which is what keeps "one React per realm" true across code the app didn't compile.
