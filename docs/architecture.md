...
- **Windowed** (Linux under Wayland — GNOME's default compositor, so the common case on Ubuntu): a single normal, decorated, resizable, taskbar-visible window, same widgets/grid/drag engine, running as `screen-0`. Wayland compositors don't grant clients absolute window positioning, always-below stacking, or click-through-by-region — the three things the overlay flow depends on — so rather than fight a protocol that won't grant them, Rust just doesn't ask: no per-monitor windows, no cursor-poller thread. Transparency isn't one of those three refused hints — Wayland compositors composite alpha-blended client surfaces natively — so this window is still `transparent(true)` like the overlay flow's: decorations (title bar, border) stay opaque, but empty grid space shows the desktop through, same look as the overlay flow minus positioning/stacking/click-through. `WIGL_MODE=windowed`/`overlay` env var overrides the auto-detection, so this flow is also available by choice on macOS or X11 for anyone who prefers a normal app window over desktop-glued widgets.
