...
**Every widget is a plugin, read off disk at startup.** `loadPlugins()` (`src/wigl/plugins/`, contract in `docs/plugins.md`) discovers `wigl-widgets/<name>/` folders installed into the app-data dir, evaluates each in the same JS realm as the host, and builds the id→component map `<Desktop>` mounts from. A plugin gets its React and every shared helper from the host through a module registry rather than bundling its own — which is what keeps "one React per realm" true across code the app didn't compile.

**A widget is not a window.** A plugin's `index.tsx` has exactly one export — a default-exported component — passing grid size/position as plain props to `<Widget w h col row>` (`src/wigl/widget.tsx`, exported from `@/wigl`). Each monitor's `<Desktop>` (`src/wigl/Desktop.tsx`) renders whichever subset of the discovered widgets is assigned to it, tiled on a shared grid (`src/wigl/grid/math.ts`). Adding a widget = adding a folder and running `bun run plugin:install` — nothing in Rust, `tauri.conf.json`, or the window layer needs to change, because widget count and window count are independent by design.
