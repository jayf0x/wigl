...
**Every widget is built and shipped through the plugin mechanism, read off disk at startup.** `loadPlugins()` (`src/wigl/plugins/`, contract in `docs/widgets.md`) discovers `wigl-widgets/<name>/` folders installed into the app-data dir, evaluates each in the same JS realm as the host, and builds the id→component map `<Desktop>` mounts from. A widget gets its React and every shared helper from the host through a module registry rather than bundling its own — which is what keeps "one React per realm" true across code the app didn't compile.

**A widget is not a window.** A widget's `index.tsx` has exactly one export — a default-exported component — passing grid size/position as plain props to `<Widget w h col row>` (`src/wigl/widget.tsx`, exported from `@/wigl`). Each monitor's `<Desktop>` (`src/wigl/Desktop.tsx`) renders whichever subset of the discovered widgets is assigned to it, tiled on a shared grid (`src/wigl/grid/math.ts`). Adding a widget = adding a folder and running `bun run widget:install` — nothing in Rust, `tauri.conf.json`, or the window layer needs to change, because widget count and window count are independent by design.
...
- **Grid + drag + resize** — `src/wigl/grid/math.ts` (pure tiling math: cell↔px conversion, collision push, gravity compaction), `src/wigl/grid/config.ts` (every tunable of the tiling desktop), and the drag/resize handling inside `src/wigl/Desktop.tsx` (pointer-driven, CSS-transform based — not native window moves). A widget's position and size are grid coordinates persisted through `useStorage` and broadcast across monitor windows over `emit`/`listen`, so a cross-monitor drag doesn't wait on a poll tick. Widgets never touch grid math or drag/resize state directly — `WidgetHeader`'s `data-drag-handle` attribute is all a widget needs to opt into dragging, and every widget is resizable from all four edges with no opt-in at all (the handles live on `Desktop.tsx`'s own wrapper around each widget, not inside the widget). Resize is local to one monitor only — unlike drag, it never hands off across screens.
...
- **Persistent storage** — `src/wigl/hooks/useStorage.ts` and `useQuery.ts` share one SQLite client (`src/wigl/storage/client.ts`) for a kv table in `wigl.db` under `storageRoot()` (`src/wigl/settings/config.ts`) — Tauri's per-OS app-data dir (`appDataDir()` — macOS: `~/Library/Application Support/<id>`, Linux: `~/.local/share/<id>`; `<id>` is `src/config/app.ts`'s `APP_IDENTIFIER`, read from `tauri.conf.json` so it can't drift out of sync) unless a Settings-modal `storage.root` override relocates it — read/written by shelling out to the system's `sqlite3` CLI (consistent with "real CLI over Rust commands" below — no Tauri SQL plugin, no Rust). `sqlite3` isn't bundled, so it's an *optional* system dependency: widgets that don't touch storage work with none installed at all; ones that do simply log read/write errors until it's installed (ships by default on macOS, `apt install sqlite3` on Ubuntu) — no install-time check, no postinstall hook, by design, to keep the app's own footprint minimal. Shared ahead of the usual two-consumer threshold deliberately, because its whole point is one DB shared by widgets *and* external CLIs (`scripts/`) — see `docs/widgets.md`'s "Persistent storage".
- **Settings** — right-click → Settings opens `src/wigl/settings/SettingsModal.tsx`, a centered dialog aggregating built-in sections (Appearance, Grid, Storage, Widgets — `src/wigl/settings/sections/`) with whatever a widget has contributed via a sibling `settingsSection.tsx` (registered by the plugin loader, see `docs/widgets.md`). Two storage tiers, same split as the rest of the app: live fields (theme, a widget's own settings, and the Grid section — it mutates `grid/config.ts`'s `TILING` in place and has `Desktop.tsx` rebuild the layout, since grid math is JS-only and Rust never reads `TILING`) go through `useStorage` same as any other widget state; restart-required fields (`storage.root`, `app.mode`) are *overrides* in one file, always at the fixed `app_data_dir()/wigl-config.json` regardless of any `storage.root` override (it's what tells every other path where to look, so it can't itself move), read/written by the `config_get`/`config_set` Rust commands in `src-tauri/src/lib.rs` — the same atomic tmp-file+rename shape as `secrets.json` (see that file's own comment for why a native command clears the "shell out unless truly impossible" bar here too). Overrides are read exactly once, in `main.tsx`, before the first render — there's no live-mutation path for a Tier-2 field, by rule (`storage.root` genuinely can't move mid-session; `app.mode` restarts itself), so a changed value only takes effect after the `@tauri-apps/plugin-process` restart the modal offers.

Everything else — data fetching, polling, local state specific to one widget's own logic — is plain component code scoped to its own `wigl-widgets/<name>/` folder. **The promotion rule**: don't extract a "shared" hook or utility until a second widget actually needs the *same* logic; one hook used by one widget is over-engineering with extra steps. When something does cross that threshold, move it into `src/wigl/` and export it from the appropriate barrel — that's the entire ceremony.
