...
3. **Looks unclickable / whole screen looks stuck**: this is almost always the click-through hit-testing, not the widget itself — `src-tauri/src/cursor.rs` toggles native click-through based on hit-rects each `<Desktop>` reports via `set_hit_rects`. A widget with a `NaN`/garbage position reports a hit-rect that never matches the cursor, so the whole window falls back to click-through everywhere (see "Renaming a `useStorage` shape..." below — this is the most common cause).
...
## Power: the app runs 24/7

Idle should cost ~nothing. Anything that wakes the process on a timer is a regression here, so the rules are: no permanent interval/poll in core (`useStorage` has none — a CLI writer must trigger its own update; the drag watchdog arms only while a gesture is live; the macOS cursor watcher is event-driven — see `docs/architecture.md`), animate with CSS transitions rather than per-frame JS (`wigl-widgets/music`'s scrubber is the pattern: a coarse JS tick sets an optimistic target, CSS moves the bar), and run a **release** build for daily use (`bun run tauri build --bundles app`; debug builds are unoptimized and cost several times more per wakeup — `verify`/`qa` build debug on purpose).

Measure before changing anything (no `sudo` needed): `top -l 6 -s 3 -pid $(pgrep -x wigl) -stats pid,cpu,idlew,power` for steady-state CPU/wakeups/power, and `sample $(pgrep -x wigl) 10 -file out.txt` for where a busy thread spends its time — a healthy idle app shows the main thread parked in `mach_msg` for ~every sample, and no `posix_spawn` (a spawn per interval means a widget or hook is shelling out on a timer). The WebKit helper processes (`com.apple.WebKit.WebContent`) should also sit at 0% CPU. To see which cursor watcher is live, launch the binary directly (stderr is not in `log show`) and look for `[wigl] cursor: event-driven` (or the fallback line) at startup.
