...
1. **Folder contract**: the widget must be `wigl-widgets/<name>/index.tsx` with a **default** export whose root is a `<Widget>` — a named-only export, a differently-named file, or a default export that doesn't render `<Widget>` all fail `bun run plugin:check` (run it before installing; the loader also logs `[wigl] plugin "<id>" ... entry has no default export` at runtime if a bad build somehow got installed anyway). Check the folder name isn't `main` (reserved), and that it matches `manifest.json`'s `id`.
