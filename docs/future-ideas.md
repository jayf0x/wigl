...
~~Sorting / multiple header actions~~ and ~~a VS Code open action~~ from the reference widget are now built (2026-07-10) — see `SORT_ACTIONS`/`SORTERS` in `src/widgets/repos/index.tsx` and `openInEditor` in `src/widgets/repos/useReposWidget.ts`.

## Decided (not just deferred)

- **Multiple concurrent widgets: one Tauri window per widget.** Chosen over rendering multiple widgets as sibling elements inside one window because `src/drag.ts` and `tauri-plugin-window-state` already operate at the window level (a window has one position; dragging moves the whole window) — one window per widget reuses both completely unchanged instead of requiring a second, incompatible in-page drag/position system. As a side effect, each widget gets its own WebView/JS realm, so render/performance isolation between widgets is structural, not something app code has to maintain (see `docs/architecture.md`'s "Render isolation"). Decided 2026-07-11.
...
- **A third widget**: `Widget`/`WidgetHeader` already generalized once (see "Decided" above) — a third widget is the point to check whether anything *else* is now duplicated identically across all three (not just two), per `docs/architecture.md`'s sharing threshold. Don't pre-extract before that.
