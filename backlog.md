...
- [ ] **F11 — music widget: confirm audio actually reaches the speakers, then wire in the visualizer seam.** The `music` widget's whole pipeline is verified headlessly — control WS auth, Sendspin proxy auth, player registration, `play_media`, MA advancing the queue, events flowing (all confirmed live against the Docker MA in `.idea/ma-data/`). The one thing no headless check covers: that `SendspinPlayer` + `player.unlock()` (called on every transport interaction) actually produces sound on this machine's output device. Someone needs to open the widget, search a station, hit play, and listen. If it's silent, first suspects: AudioContext never resumed (unlock not firing from the gesture), WKWebView codec support (the widget offers opus/flac/pcm and the SDK filters — pcm should always survive), or MA's stream format negotiation. Once audio is confirmed, the visualizer seam is ready to build: sendspin-js emits `DecodedAudioChunk` (raw PCM Float32 per channel) — a `<canvas>` bars/scope reading that, no `AnalyserNode`/`<audio>` needed. See `todo-musicplayer.md` "Phase 0 → Audio".
- [ ] **F12 — music widget: add YouTube Music (owner has to sign in).** The widget ships against RadioBrowser (no credentials). YouTube Music needs a Google account added to Music Assistant itself: MA UI (`http://127.0.0.1:8095`) → Settings → Providers → YouTube Music → sign in. Then in the widget's Settings section, set "Search provider" to `ytmusic` (or leave blank to search everything). No widget code change — search/results/play are provider-agnostic. This entry is just the reminder that the brief's "one source to start: YouTube Music" is one owner action away, not done.
