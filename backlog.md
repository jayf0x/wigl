...
- [ ] **B17 — Stale backlog-ticket-id references scattered across code comments.** Several files cite a ticket id that's since been deleted from this file (this file's own rule: fixed entries get deleted, not archived) — e.g. `wigl-widgets/LocalCode/serverProcess.ts`/`serverLock.ts` mention "B7", `src/wigl/plugins/registry.ts` mentions "B12", `docs/principles.md` mentions "B15", several `wigl-widgets/LocalCode` files mention "B1"/"B2"/"B4". Each comment still reads fine on its own (the id is decoration, not load-bearing), but the cross-reference itself resolves to nothing today. Fix: sweep these and either drop the "(backlog.md BN)" parenthetical entirely or replace it with the commit hash that landed the fix, matching the convention in `AGENTS.md`'s "Testing" section.
