...
- [ ] **F11 — music widget: confirm audio reaches the speakers; then wire the visualizer seam.** Everything up to "MA streams PCM to a client authenticated as this widget" is verified by `wigl-widgets/music/tests/music.e2e.test.ts` + a live playback check (player registers, queue advances, MA sets output protocol to Native). The DAC hop is browser-only — `wigl-widgets/music/tests/audio-check.md` is the by-ear checklist. Round 2 moved output to a hidden `<audio>` element (media-element mode) with `codecs:["pcm"]` — the direct-AudioContext path was the prime silence suspect. If still silent, that doc's devtools steps (`audio[data-wigl-music]` state) are the next move. Once confirmed: sendspin-js emits `DecodedAudioChunk` (Float32 PCM per channel) or the `<audio>` element can feed `createMediaElementSource` → a `<canvas>` scope.
- [ ] **F12 — music widget: YouTube Music needs a one-time Google sign-in.** The widget shows a "＋ add youtube music" footer (opens the MA UI at `http://127.0.0.1:8095`, login `test`/`testtest`) whenever the `ytmusic` provider isn't enabled; adding it there makes it show up in search automatically (the footer disappears). No widget code change — the whole search/play path is provider-agnostic. This entry just tracks that the brief's "YouTube Music" is gated on that sign-in, which only the owner can do.
