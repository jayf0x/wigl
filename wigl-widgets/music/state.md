...
| `components/QueueList.tsx` | The Queue tab list (items after the current one) with pointer drag-reorder (P0.5: DOM order stays fixed during a drag — rows move by CSS transform so pointer capture never drops; list frozen to a snapshot for the gesture; one `moveQueueItem` on drop, `arrayMove` in `util.ts` is the shared reducer) + per-row `⋯`. `QueueHeader`: "Save" (→ `saveQueueAsPlaylist`, queue untouched; on success swaps to a flashing "saved as …" acknowledgement — feedback D) + two-step "Clear" (D2/D4). |
