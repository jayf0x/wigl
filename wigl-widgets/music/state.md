...
| `components/DetailView.tsx` | `ArtistView` / `AlbumView` / `PlaylistView` — the nav-stack destinations. `PlayPills` = a "Play" button (always plays now; keep-tail vs wipe follows the D1 toggle) + an explicit "Add to queue" in append mode. `PlaylistView`: inline Rename (E1), Background image (E3), two-step Delete, and drag-reorder of the track list (P6.2 — `useDragReorder` + `api.reorderPlaylist`, optimistic `order` state cleared on the next fetch). Track rows capped at `PLAYLIST_RENDER_CAP`. |
