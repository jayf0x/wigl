...
| `serverProcess.ts` | Docker ops via `runCmd("sh", …)`: `maReachable`, `startMaContainer` (auto-start), `dockerState` (up / daemon-down / no-docker) + `startDockerDesktop` (P4 — `open -a Docker` / `systemctl --user`), and H1's `restartMaContainer` / `clearMaCache` / `updateMaImage`. Docker binary discovered from a small candidate list. |
| `components/OfflinePanel.tsx` | P4 — the unreachable-server surface: shared `ErrorOverlay` + a "Start server" button (`api.startServer` — starts Docker if the daemon is down, then the container, then reconnects), a retry, and the auto-start checkbox in-panel. |
...
| `components/Home.tsx` | Tabbed default pane: Queue (`QueueList`) / Playlists / Recent / Browse (`BrowseTab`). `PinnedStrip` above the tabs = horizontal quick-access chips for `api.pinnedPlaylists` (F1). |
| `components/useDragReorder.ts` | Shared pointer drag-to-reorder hook (P0.5 / P6.2): frozen list snapshot for the gesture, DOM order fixed (rows move by CSS transform so pointer capture never drops), `onDrop(from, to, item)` once on release. Backs both `QueueList` and `PlaylistView`'s track list. |
| `components/QueueList.tsx` | The Queue tab list (items after the current one), `useDragReorder` + one `moveQueueItem` on drop, per-row `⋯`. `QueueHeader`: "Save" (→ `saveQueueAsPlaylist`, queue untouched; on success swaps to a flashing "saved as …" acknowledgement — feedback D) + two-step "Clear" (D2/D4). |
| `components/DetailView.tsx` | `ArtistView` / `AlbumView` / `PlaylistView` — the nav-stack destinations. `PlayPills` = a "Play" button (always plays now; keep-tail vs wipe follows the D1 toggle) + an explicit "Add to queue" in append mode. `PlaylistView`: title is an `<InlineEdit>` (P6.3 → `api.renamePlaylist`), background image edited on hover of the cover thumbnail (P6.4 → `api.setPlaylistImage`), two-step Delete, drag-reorder of the track list (P6.2 — `useDragReorder` + `api.reorderPlaylist`). Track rows capped at `PLAYLIST_RENDER_CAP`. |
...
| `components/VFader.tsx` | A vertical, pointer-driven fader (rail + cap + detent line + scale marks). `onChange` continuous, `onCommit` on release. Keyboard ↑/↓. `data-no-drag`, `mx-nodrag-select` + `preventDefault` on down (P3.2), theme tokens. Used by `EffectsTab` (P3.1 — centred fixed-width cluster); not shared to core (single consumer). |
...
| Playlist reorder (P6.2) | **no reorder command** — builtin provider is append + remove-by-position only. `reorderPlaylist` = `remove_playlist_tracks {positions_to_remove:[1..N]}` then `add_playlist_tracks {uris: newOrder}`. Full rewrite; fine at the render cap. |
...
- No "disable artwork/previews" setting (P8.2, rejected). Art is already
  `<img loading=lazy fetchpriority=low>`; there's no measured fetch-cost
  problem to solve. Revisit only if one shows up.
- Live search waits for 3+ characters (P8.1). Empty clears; explicit
  submit / history chips run at any length.
