...
export const sourceDirExists = async (path: string): Promise<boolean> => {
  const out = await runCmd("sh", ["-c", `[ -d ${shQuote(path)} ] && echo yes`]);
...
const runGh = async (argsString: string) => {
  ...
  for (const gh of candidates) {
    const out = await runCmd("sh", ["-c", `${shQuote(gh)} ${argsString}`]);
...
export const loadArchivedRepoNames = async (): Promise<string[]> => {
...
// is present.
export const loadRemoteRepos = async (): Promise<RemoteRepo[]> => {
...
const run = async (cmd: string) => {
  const out = await runCmd("sh", ["-c", cmd]);
...
export const commitAllChanges = async ({ path }: ProjectStatus, message: string) => {
...
export const revealInFileManager = async ({ path }: ProjectStatus) => {
...
export const openInEditor = async ({ path }: ProjectStatus) => {
  if (await isMacos()) {
    try {
...
export const openInGithubDesktop = async ({ path }: ProjectStatus) => {
  if (await isMacos()) {
    try {
